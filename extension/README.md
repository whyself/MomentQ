...
ASR 凭据只存在于 companion 进程。数据流向必须知悉：百度云模式下，标签页音频会由 companion 实时上传百度智能云做语音识别（本机不落盘）；本地 Whisper 模式则完全在本机推理，但首次使用需从 HuggingFace 下载 0.26–1.6GB 模型权重。Native Messaging 路径仍未使用。
